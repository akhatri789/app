...
class CustomerController extends Controller
{
    ...
    public function delivery_status_check($zipcode){
        $deliveryStatus = DeliveryStatus::where('zip_code',$zipcode);
        
        if($deliveryStatus->count()){
            $deliveryStatus = $deliveryStatus->first();
            //'Get it by Fri, Oct 07'
            $delivaryDate = date('D, M d',strtotime('+'.$deliveryStatus->no_of_day.'days'));
            return response()->json(["status"=>true,"message"=>"Get it by ".$delivaryDate],200);
        }else{
                return response()->json(['message' => 'Pincode not available','status'=>false], 200);
        }
    }
}
