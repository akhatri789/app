...
class WebController extends Controller
{
    ...
    public function home()
    {
        ...
        $featured_products = Product::with(['reviews'])->active()
            ...
            ->get();
        //end
// Changes 13-09-2022 by amit take(12) from take(8)
    ...
    {
        // zipcode validation.
        $zipcode = DB::table('shipping_addresses')->select('zip')->where('customer_id',auth('customer')->id())->limit(1)->orderBy('id','desc')->get()->first();
        if($zipcode){
           $zipcount = DeliveryStatus::where('zip_code',$zipcode->zip)->where('status',1)->count();
            if($zipcount == 0){
                Toastr::error("Delivery not available on given zipcode.");
                return back();
            }
        }
    ...
    public function products(Request $request)
    {
        ...
        if ($request['data_from'] == 'category') {
            ...
            foreach ($products as $product) {
                ...
                $categorySlugs = Category::select('slug')->whereIn('id',$categoryIds)->get()->toArray();
        ...
        if ($request['sort_by'] == 'latest') {
            ...
            $fetched = $query->orderBy('after_discount_price', 'ASC');
        } elseif ($request['sort_by'] == 'high-low') {
            $fetched = $query->orderBy('after_discount_price', 'DESC');
        ...
        if ($request['price_range'] != null) {
            ...
            //$fetched = $fetched->addSelect(DB::raw("(CASE WHEN discount_type = 'percent' AND CAST(discount AS FLOAT) >= 0.00  THEN (((CAST(discount AS FLOAT)/100)*unit_price)+unit_price) ELSE (CAST(discount AS FLOAT)+unit_price) END) AS withdiscount")); 
        ...
        if ($request['data_from'] == 'category') {
            $data['brand_name'] = Category::where('slug',$request['slug'])->first()->name;
        }
        /*if ($request['data_from'] == 'brand') {
        ...
        if ($request['data_from'] == 'color') {
            $color_data = Color::find((int)$request['id']);
            if($color_data) {
                $data['color_name'] = $color_data->name;
            }else {
                Toastr::warning(translate('not_found'));
                return redirect('/');
            }
        ...
        $colorList = Color::select('id','code','name')->whereIn('code',$colorListArr)->get()->toArray();
        return view('web-views.products.view', compact('products', 'data','colorList','variants'), $data);
    }
 ...
 public function medicine_surgery()
    {
        $medicine_surgery = BusinessSetting::where('type', 'medicine_surgery')->first();
        return view('web-views.medicine_surgery', compact('medicine_surgery'));
    }
    ...
    public function zipcodeCheck($zipcode){
        ...
        $zipcount = DeliveryStatus::where('zip_code',$zipcode)->where('status',1);
        if($zipcount->count() == 0){
            return response()->json(["message"=>"Delivery not available on given zipcode.","status"=>false]);
        }else{
            $zipcount = $zipcount->first();
            //'Get it by Fri, Oct 07'
            $delivaryDate = date('D, M d',strtotime('+'.$zipcount->no_of_day.'days'));
            return response()->json(["status"=>true,"message"=>"Get it by ".$delivaryDate]);
        }
    }
}
