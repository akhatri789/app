...
class Helpers
{
    ...
    public static function send_push_notif_to_topic($data)
    {
        ...
        $to = array();
        if(in_array("all",explode(",",$data->users))){
            $tokenlist = DB::table('users')->select("cm_firebase_token")->get()->toArray();
            foreach($tokenlist as $tl){
                if($tl->cm_firebase_token != "" && $tl->cm_firebase_token != null){
                    array_push($to,$tl->cm_firebase_token);
                }  
            }
        }else{
            $tokenlist = DB::table('users')->select("cm_firebase_token")->whereIn('id',explode(",",$data->users))->get()->toArray();
            foreach($tokenlist as $tl){
               if($tl->cm_firebase_token != "" && $tl->cm_firebase_token != null){
                    array_push($to,$tl->cm_firebase_token);
                }  
...
//            dd($str);
    ...
    public static function get_convenience_fee(){
        $config = [];
        $data = BusinessSetting::where(['type' => 'convenience_fee'])->first();
        if (isset($data)) {
            $config = (array)json_decode($data['value'], true);
            if (is_null($config)) {
                $config = $data['value'];
            }
        }
